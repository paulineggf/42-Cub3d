...
/*   Updated: 2019/11/28 18:12:32 by pganglof         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	*(float*)wall->unitx = (float)abs((int)(*(float*)(wall->unitx)
	+ (int)(BLOCK_SIZE / tan(gamer->degree * M_PI / 180))));
	if (gamer->facing_up)
		*(float*)wall->unity = (float)abs((int)(*(float*)(wall->unity) + -BLOCK_SIZE));
	else
		*(float*)wall->unity = (float)abs((int)(*(float*)(wall->unity) + BLOCK_SIZE));
...
{
	if (gamer->facing_up)
		*(float*)(wall_hor->unity) = (float)abs((int)((*(int*)(gamer->unity)
		/ BLOCK_SIZE) * (BLOCK_SIZE) - 1));
	else
		*(float*)(wall_hor->unity) = (float)abs((int)(*(int*)(gamer->unity) / BLOCK_SIZE)
		* (BLOCK_SIZE) + BLOCK_SIZE);
	*(float*)(wall_hor->unitx) = (float)abs((int)(*(int*)(gamer->unitx)
	...
	while (wall_hor->posy >= 0 && wall_hor->posx >= 0 && map->map[wall_hor->posy][wall_hor->posx] == 0)
...
static void		fill_wall_ver(t_map *map, t_pos *gamer, t_pos *wall_ver)
{
	if (gamer->facing_right)
		*(float*)(wall_ver->unitx) = (float)abs((int)((*(int*)(gamer->unitx)
		/ BLOCK_SIZE) * (BLOCK_SIZE) + BLOCK_SIZE));
	else
		*(float*)(wall_ver->unitx) = (float)abs((int)((*(int*)(gamer->unitx)
		/ BLOCK_SIZE) * (BLOCK_SIZE) - 1));
	*(float*)(wall_ver->unity) = (float)abs((int)(*(int*)(gamer->unity) + (*(int*)(gamer->unitx)
	...
	{
		//if (gamer->facing_right)
			*(float*)wall_ver->unitx = (float)abs((int)(*(float*)wall_ver->unitx + BLOCK_SIZE));
		//else
		//	*(float*)wall_ver->unitx = *(float*)wall_ver->unitx + -BLOCK_SIZE;
		*(float*)wall_ver->unity = (float)abs((int)(*(float*)wall_ver->unity
		- (int)(BLOCK_SIZE * tan(gamer->degree * M_PI / 180))));
		wall_ver->posx = *(float*)(wall_ver->unitx) / BLOCK_SIZE;
		wall_ver->posy = *(float*)(wall_ver->unity) / BLOCK_SIZE;
	}
